...
const DEFAULT_VISI =
    '"Profesional dan Proporsional dalam penyusunan produk hukum daerah, pemberian bantuan hukum dan hak asasi manusia, pelayanan informasi dan dokumentasi hukum serta pengawasan produk hukum daerah kabupaten/kota"';
...
export default function VisiMisi({ item }: { item?: any }) {
    const data = (item?.content && typeof item.content === 'object') ? item.content : {};

    const visi: string = data.visi ?? DEFAULT_VISI;
    const misi: string[] = normaliseList(data.misi).length > 0
        ? normaliseList(data.misi)
        : DEFAULT_MISI;
