...
/* ------------------------------------------------------------------ */
/* Helper: build diagram list from DB content                          */
/* ------------------------------------------------------------------ */
function buildDiagrams(content: any): Diagram[] {
    if (!content || typeof content !== 'object') return DEFAULT_DIAGRAMS;

    const raw = content.diagrams;
    if (!Array.isArray(raw) || raw.length === 0) return DEFAULT_DIAGRAMS;

    return raw.map((d: any, i: number) => ({
        id: i + 1,
        // image_path from FileUpload is stored as storage path -> prepend /storage/
        src: d.image_path
            ? (d.image_path.startsWith('http') ? d.image_path : `/storage/${d.image_path}`)
            : (DEFAULT_DIAGRAMS[i]?.src ?? '/images/struktur1.webp'),
        title: d.title ?? DEFAULT_DIAGRAMS[i]?.title ?? `Bagan ${i + 1}`,
        subtitle: d.subtitle ?? DEFAULT_DIAGRAMS[i]?.subtitle ?? '',
        description: d.description ?? DEFAULT_DIAGRAMS[i]?.description ?? '',
        highlights: Array.isArray(d.highlights)
            ? d.highlights.map((h: any) =>
                typeof h === 'string' ? h : (h?.value ?? String(h))
            ).filter(Boolean)
            : (DEFAULT_DIAGRAMS[i]?.highlights ?? []),
    }));
}
...
export default function StrukturOrganisasi({ item }: { item?: any }) {
    ...
    const diagrams = buildDiagrams(item?.content);

    const currentIdx = active !== null ? diagrams.findIndex((d) => d.id === active) : -1;
    const currentDiagram = currentIdx >= 0 ? diagrams[currentIdx] : null;

    function goPrev() {
        if (currentIdx > 0) setActive(diagrams[currentIdx - 1].id);
    }
    function goNext() {
        if (currentIdx < diagrams.length - 1) setActive(diagrams[currentIdx + 1].id);
    }
        ...
        <PublicLayout>
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="text-center mb-10">
                        ...
                        <p className="text-slate-500 text-sm max-w-xl mx-auto">
                            Klik gambar untuk memperbesar. Terdapat {diagrams.length} bagan struktur yang menggambarkan
            ...
            {currentDiagram && (
                <Lightbox
                    ...
                    hasNext={currentIdx < diagrams.length - 1}
                />
            )}
        </PublicLayout>
    );
